    ...
    commands
        .spawn((
            NodeBundle {
                ...
                background_color: Color::srgba(0.1, 0.1, 0.3, 0.9).into(),
                ..default()
            },
            TowerUpgradePanel,
        ))
            ...
            parent
                .spawn((
                    ButtonBundle {
                        ...
                        background_color: Color::srgb(0.5, 0.8, 0.5).into(),
                        ..default()
                    },
                    UpgradeButton,
                ))
...
fn create_tower_type_button(parent: &mut ChildBuilder, tower_type: TowerType, text: &str) {
    parent
        .spawn((
            ButtonBundle {
                style: Style {
...
pub fn update_upgrade_panel_system(
    ...
    mut panel_query: Query<&mut Style, With<TowerUpgradePanel>>,
    ...
    if let Ok(mut panel_style) = panel_query.get_single_mut() {
        panel_style.display = if selection_state.upgrade_panel_visible {
            Display::Flex
        } else {
            Display::None
        };
    ...
    if let Some(tower_entity) = selection_state.selected_tower_entity {
        if let Ok(tower_stats) = towers_query.get(tower_entity) {
            // Update tower info
            if let Ok(mut text) = tower_info_query.get_single_mut() {
            ...
            if let Ok(mut text) = current_stats_query.get_single_mut() {
            ...
            if let Ok(mut text) = upgrade_preview_query.get_single_mut() {
                if tower_stats.can_upgrade() {
                    ...
                    text.sections[0].value = "Max level reached!".to_string();
                }
            }

            // Update upgrade cost
            if let Ok(mut text) = upgrade_cost_query.get_single_mut() {
                if tower_stats.can_upgrade() {
            ...
            if let Ok(mut text) = upgrade_button_query.get_single_mut() {
                if let Ok(mut color) = upgrade_button_style_query.get_single_mut() {
                    if tower_stats.can_upgrade() {
                        let cost = tower_stats.get_upgrade_cost();
                        if economy.can_afford(&cost) {
                            ...
                            *color = Color::srgb(0.5, 0.8, 0.5).into(); // Green
                            ...
                            text.sections[0].value = "CAN'T AFFORD".to_string();
        ...
        if let Ok(mut text) = tower_info_query.get_single_mut() {
            text.sections[0].value = "Select a tower to upgrade".to_string();
        }
        if let Ok(mut text) = current_stats_query.get_single_mut() {
            text.sections[0].value = "".to_string();
        }
        ...
        if let Ok(mut text) = upgrade_button_query.get_single_mut() {
            text.sections[0].value = "SELECT TOWER".to_string();
        }
    }
}